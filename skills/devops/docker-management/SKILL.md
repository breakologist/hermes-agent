...
| Container can't reach host service | `host.docker.internal` doesn't exist in `network_mode: host` | Use `localhost` — with host networking, container shares the host's network stack |
| Container can't reach host service (bridge) | Using `localhost` in container | Use `host.docker.internal` — only works in bridge (default) networking |
