...
### Zone Data (Brief)
### Zone Metadata (Full — Qliphoth import)

**Source:** `zones.ts` from [lumpenspace/ccru](https://github.com/lumpenspace/ccru) (2026-04-30).  
Full dict in `references/zone_metadata_full.py`. Individual wiki pages: [[zone-0]] … [[zone-9]].

Provides: `planet`, `planetFull`, `desc`, `spinal`, `meshTag`, `door`,
`phaseCount`, `lemurs` (list), `lemurian`, `centauri`.

```python
from references.zone_metadata_full import ZONE_META
earth_meta = ZONE_META[3]
print(earth_meta['lemurian'])  # → "Swarming insectoid reversion..."
```

### Zone Metadata (Full — Qliphoth import)

**Source:** `zones.ts` from [lumpenspace/ccru](https://github.com/lumpenspace/ccru) (snapshot 2026-04-30).  
Individual zone pages: [[zone-0]] … [[zone-9]].

```python
# Full metadata including planetary, lore, doors, lemurs, centauri
ZONE_META = {
    0: {
    ...
    # ... zones 2–8 omitted here for brevity; see references/zone_metadata_full.py
    9: {
...
**Helper:**
```python
def get_zone_meta(zone: int) -> dict:
    """Return full metadata dict for a zone (from ZONE_META)."""
