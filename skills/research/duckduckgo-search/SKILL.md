...
- **Availability**: DuckDuckGo actively blocks requests from cloud/VPS IPs (not just rate-limits — returns `AttributeError: 'NoneType' object has no attribute 'replace'`). If running on a VPS, cloud VM, or datacenter IP, DuckDuckGo will almost certainly fail. Use SearXNG or a paid API instead. On residential IPs (home machine, laptop), it works fine.
...
| Search returns `AttributeError: 'NoneType' ... replace` | DuckDuckGo blocked this server's IP (common on cloud/VPS) | Won't fix from this IP — use SearXNG, Serper, or browser search instead |
| Search returns empty (no error) | Rate limiting, poor query, or transient issue | Wait a few seconds, retry, or adjust the query |
